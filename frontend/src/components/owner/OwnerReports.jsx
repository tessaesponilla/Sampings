...
const OwnerReports = () => {
  ...
  const generatePDF = async () => {
    ...
    // Logo centered (only add if successfully loaded)
    if (logoImg.complete && logoImg.naturalWidth > 0) {
      doc.addImage(logoImg, 'PNG', pageW/2 - 12, 6, 24, 24);
    }
    ...
    if (insights) {
      ...
      kpis.forEach((kpi, i) => {
        ...
        doc.setFontSize(7);
        doc.text(kpi.label, x, yPos + 18);
        doc.setTextColor(15, 25, 60);
        doc.setFontSize(12);
        doc.setFont('helvetica', 'bold');
        doc.text(kpi.value, x, yPos + 30);
        doc.setFont('helvetica', 'normal');
      });
      ...
      doc.setFontSize(9);
      doc.setFont('helvetica', 'bold');
      doc.text('ORDER TYPE BREAKDOWN', margin + 5, yPos + 8);
      
      // Online Orders
      doc.setFontSize(7);
      doc.setTextColor(120, 125, 135);
      doc.text('Online Orders', margin + 5, yPos + 18);
      doc.setFontSize(11);
      doc.setTextColor(99, 102, 241);
      doc.setFont('helvetica', 'bold');
      doc.text(String(metrics.onlineCount), margin + 5, yPos + 28);
      doc.setFontSize(7);
      doc.setTextColor(80, 85, 95);
      doc.setFont('helvetica', 'normal');
      doc.text(`PHP ${metrics.onlineRevenue.toLocaleString()}`, margin + 5, yPos + 35);

      // Walk-in Orders
      doc.setFontSize(7);
      doc.setTextColor(120, 125, 135);
      doc.text('Walk-in Orders', margin + 40, yPos + 18);
      doc.setFontSize(11);
      doc.setTextColor(245, 158, 11);
      doc.setFont('helvetica', 'bold');
      doc.text(String(metrics.walkinCount), margin + 40, yPos + 28);
      doc.setFontSize(7);
      doc.setTextColor(80, 85, 95);
      doc.setFont('helvetica', 'normal');
      doc.text(`PHP ${metrics.walkinRevenue.toLocaleString()}`, margin + 40, yPos + 35);
      ...
      doc.setFontSize(9);
      doc.setFont('helvetica', 'bold');
      doc.text('STATUS BREAKDOWN', rightBoxX + 5, yPos + 8);
      ...
      statuses.forEach(s => {
        ...
        doc.setFontSize(7);
        doc.setTextColor(80, 85, 95);
        doc.text(s.label, rightBoxX + 5, statusY + 3);
        doc.text(`${count}`, rightBoxX + 35, statusY + 3);
        doc.setFillColor(...s.color);
        doc.rect(rightBoxX + 42, statusY, barW, 4, 'F');
        statusY += 6;
      });
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
        <h2 className="bebas" style={{ fontSize: '28px', margin: 0, color: 'var(--navy)' }}></h2>
      ...
      <div className="card" style={{ padding: '24px' }}>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
            ...
            <tbody>
              {orders.length === 0 ? <tr><td colSpan={9} style={{ textAlign: 'center', padding: '40px', color: 'var(--muted)' }}>No orders for this period.</td></tr> :
                orders.map(o => (
...
<div style={{ height: '24px' }}></div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
        <h3 style={{ fontWeight: 700, fontSize: '16px', margin: 0, color: 'var(--navy)' }}>Order Type Breakdown</h3>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '24px' }}>
        <div className="card" style={{ padding: '20px' }}>
          <div style={{ fontSize: '11px', fontWeight: 600, color: 'var(--muted)', textTransform: 'uppercase', marginBottom: '12px' }}>Online Orders</div>
          <div style={{ fontSize: '24px', fontWeight: 700, color: 'var(--navy)', marginBottom: '8px' }}>{metrics.onlineCount}</div>
          <div style={{ fontSize: '13px', color: 'var(--muted)' }}>Revenue: <span style={{ fontWeight: 600, color: 'var(--text)' }}>₱{metrics.onlineRevenue.toLocaleString()}</span></div>
        </div>
        <div className="card" style={{ padding: '20px' }}>
          <div style={{ fontSize: '11px', fontWeight: 600, color: 'var(--muted)', textTransform: 'uppercase', marginBottom: '12px' }}>Walk-in Orders</div>
          <div style={{ fontSize: '24px', fontWeight: 700, color: 'var(--amber)', marginBottom: '8px' }}>{metrics.walkinCount}</div>
          <div style={{ fontSize: '13px', color: 'var(--muted)' }}>Revenue: <span style={{ fontWeight: 600, color: 'var(--text)' }}>₱{metrics.walkinRevenue.toLocaleString()}</span></div>
        </div>
      </div>
